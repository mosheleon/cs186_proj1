...
public class JoinOptimizer {
	...
	public static int estimateTableJoinCardinality(Predicate.Op joinOp,
		...
		int card = 1;
		// some code goes here
		TableStats ts1 = stats.get(table1Alias);
		TableStats ts2 = stats.get(table2Alias);
		if (joinOp == Op.EQUALS) {
			if (t1pkey && !t2pkey) {
				card = card2;
			}
			...
			card = (int)Math.floor(0.3 * card1 * card2);
		}
	...
	public Vector<LogicalJoinNode> orderJoins(
		...
		for (int i = 1; i <= joins.size(); i++) {
			...
			for (Iterator<Set<LogicalJoinNode>> it1 = subsets.iterator(); it1.hasNext();) {
				Set<LogicalJoinNode> s = it1.next();
				double bsf = Double.MAX_VALUE;
				CostCard best = null;
				for (Iterator<LogicalJoinNode> it2 = s.iterator(); it2.hasNext();) {
					Set<LogicalJoinNode> s_prime = new HashSet<LogicalJoinNode>();
					s_prime.addAll(s);
					LogicalJoinNode n = it2.next();
					s_prime.remove(n);
					CostCard next = computeCostAndCardOfSubplan(stats, filterSelectivities, n, s, bsf, cache);
					if (next != null && (best == null || next.cost < best.cost)) {
						best = next;
		...
		if (cache.getOrder(j) == null)
			return joins;
		return cache.getOrder(j);
	}
